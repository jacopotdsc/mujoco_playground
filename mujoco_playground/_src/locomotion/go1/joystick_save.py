...
class Joystick(go1_base.Go1Env):
  ...
  def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
    ...
    def substep_fn(data, _):
        ...
        tau_pd = 60.0 * ( target_pos - data.qpos[7:]) + 1.0 * (-data.qvel[6:])
    ...
    # --- Whole-body controller: run every step (like srbd_quad) ---
    
    #tau_ff = self._whole_body_ctrl(data.qpos, data.qvel, state.info["mpc_state"])
    #state.info["mpc_tau_ff"] = tau_ff

    #target_pos = self._default_pose + action * self._config.action_scale
    #tau_pd = 60.0 * ( target_pos - state.data.qpos[7:]) + 1.0 * (-state.data.qvel[6:])
    ...
    #target_pos = self._default_pose + action * self._config.action_scale
    #motor_targets = 35*(target_pos - state.data.qpos[7:]) + 0.5 * ( -state.data.qvel[6:])
    data = mjx_env.step(
